...
void test_read_source_code() {
    ...
    run_test("test reading from file line 0", equal(get_code_line(&source_code), "some\n"));
    ...
    move_to_next_line(&source_code);
    run_test("test reading from file line 3", equal(get_code_line(&source_code), "    here.\n"));
